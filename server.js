        ...
        if (streams.length > 0) {
          return {
            sources: streams.map((url, i) => ({
              url,
              quality: url.includes('1080') ? '1080p' : url.includes('720') ? '720p' : `Source ${i + 1}`,
...
app.get('/movie/:tmdbId', async (req, res) => {
  ...
  try {
    const data = await fetchSourcesForTmdb(tmdbId);
    if (data.sources.length === 0) {
      return res.status(404).json({ success: false, error: 'No sources found', sources: [], subtitles: [] });
    }
    res.json({ success: true, ...data });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message, sources: [], subtitles: [] });
  }
});
...
app.get('/tv/:tmdbId/:season/:episode', async (req, res) => {
  ...
  try {
    const data = await fetchSourcesForTmdb(tmdbId, Number(season), Number(episode));
    if (data.sources.length === 0) {
      return res.status(404).json({ success: false, error: 'No sources found', sources: [], subtitles: [] });
    }
    res.json({ success: true, ...data });
